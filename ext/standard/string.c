...
int php_tag_find(char *tag, zend_string_size len, char *set);
...
/* {{{ php_bin2hex
 */
static char *php_bin2hex(const unsigned char *old, const zend_string_size oldlen, zend_string_size *newlen)
{
	register unsigned char *result = NULL;
	zend_string_size i, j;
...
/* {{{ php_hex2bin
 */
static char *php_hex2bin(const unsigned char *old, const zend_string_size oldlen, zend_string_size *newlen)
{
	zend_string_size target_length = oldlen >> 1;
 ...
 */
static inline int php_charmask(unsigned char *input, zend_string_size len, char *mask TSRMLS_DC)
{
	unsigned char *end;
	unsigned char c;
	int result = SUCCESS;
...
static void php_do_trim(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	char *str;
	char *what = NULL;
	zend_string_size str_len, what_len = 0;
...
PHP_FUNCTION(wordwrap)
{
	...
	char *newtext;
	zend_string_size textlen, breakcharlen = 1, newtextlen, chk;
	size_t alloced;
	...
	if (breakcharlen == 1 && !docut) {
		...
		if (linelength > 0) {
			chk = (zend_string_size)(textlen/linelength + 1);
		...
		for (current = 0; current < textlen; current++) {
			if (chk <= 0) {
				alloced += (zend_string_size) (((textlen - current + 1)/linelength + 1) * breakcharlen) + 1;
				newtext = erealloc(newtext, alloced);
				chk = (zend_string_size) ((textlen - current)/linelength) + 1;
			}
...
PHPAPI void php_explode(zval *delim, zval *str, zval *return_value, long limit)
{
	...
	p1 = Z_STRVAL_P(str);
	p2 = php_memnstr(Z_STRVAL_P(str), Z_STRVAL_P(delim), Z_STRSIZE_P(delim), endp);

	if (p2 == NULL) {
		add_next_index_stringl(return_value, p1, Z_STRSIZE_P(str), 1);
	} else {
		do {
			add_next_index_stringl(return_value, p1, p2 - p1, 1);
			p1 = p2 + Z_STRSIZE_P(delim);
		} while ((p2 = php_memnstr(p1, Z_STRVAL_P(delim), Z_STRSIZE_P(delim), endp)) != NULL &&
				 --limit > 1);
...
{
	char *str, *delim;
	zend_string_size str_len = 0, delim_len = 0;
...
PHPAPI void php_implode(zval *delim, zval *arr, zval *return_value TSRMLS_DC)
{
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(arr), (void **) &tmp, &pos) == SUCCESS) {
		switch ((*tmp)->type) {
			case IS_STRING:
				smart_str_appendl(&implstr, Z_STRVAL_PP(tmp), Z_STRSIZE_PP(tmp));
				break;
			...
			case IS_OBJECT: {
				...
				smart_str_appendl(&implstr, Z_STRVAL(expr), Z_STRSIZE(expr));
				if (copy) {
					zval_dtor(&expr);
				}
			}
			...
			default:
				...
				smart_str_appendl(&implstr, Z_STRVAL(tmp_val), Z_STRSIZE(tmp_val));
				zval_dtor(&tmp_val);
				break;
...
{
	char *str, *tok = NULL;
	zend_string_size str_len, tok_len = 0;
	zval *zv;
...
PHP_FUNCTION(basename)
{
	...
	zend_string_size   string_len, suffix_len = 0;
	zend_string_size ret_len;
...
PHPAPI size_t php_dirname(char *path, zend_string_size len)
{
...
/* {{{ php_strspn
 */
PHPAPI zend_string_size php_strspn(char *s1, char *s2, char *s1_end, char *s2_end)
{
...
/* {{{ php_strcspn
 */
PHPAPI zend_string_size php_strcspn(char *s1, char *s2, char *s1_end, char *s2_end)
{
...
PHP_FUNCTION(stristr)
{
	zval *needle;
	char *haystack;
	zend_string_size haystack_len;
	char *found = NULL;
	zend_string_size  found_offset;
	char *haystack_dup;
	char needle_char[2];
	zend_bool part = 0;
...
PHP_FUNCTION(strstr)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		found = php_memnstr(haystack, Z_STRVAL_P(needle), Z_STRSIZE_P(needle), haystack + haystack_len);
	} else {
		if (php_needle_char(needle, needle_char TSRMLS_CC) != SUCCESS) {
			RETURN_FALSE;
		}
...
PHP_FUNCTION(stripos)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		found = php_memnstr(haystack_dup + offset, needle_dup, Z_STRSIZE_P(needle), haystack_dup + haystack_len);
	} else {
		if (php_needle_char(needle, needle_char TSRMLS_CC) != SUCCESS) {
			efree(haystack_dup);
			RETURN_FALSE;
...
PHP_FUNCTION(strrpos)
{
	...
	zend_string_size needle_len, haystack_len;
	long offset = 0;
...
/* {{{ php_chunk_split
 */
static char *php_chunk_split(char *src, int srclen, char *end, zend_string_size endlen, zend_string_size chunklen, zend_string_size *destlen)
{
	char *dest;
	char *p, *q;
	zend_string_size chunks; /* complete chunks! */
	zend_string_size restlen;
	zend_string_size out_len;
...
PHP_FUNCTION(substr_replace)
{
	...
	if (Z_TYPE_PP(str) == IS_STRING) {
		if (
			...
			RETURN_STRINGL(Z_STRVAL_PP(str), Z_STRSIZE_PP(str), 1);
		}
	...
	if (Z_TYPE_PP(str) != IS_ARRAY) {
		if (Z_TYPE_PP(from) != IS_ARRAY) {
			 ...
			 */
			if (f < 0) {
				f = Z_STRSIZE_PP(str) + f;
				if (f < 0) {
					f = 0;
				}
			} else if (f > Z_STRSIZE_PP(str)) {
				f = Z_STRSIZE_PP(str);
			}
			/* if "length" position is negative, set it to the length
			 * needed to stop that many chars from the end of the string
			 */
			if (l < 0) {
				l = (Z_STRSIZE_PP(str) - f) + l;
				if (l < 0) {
					l = 0;
				}
			}

			if (f > Z_STRSIZE_PP(str) || (f < 0 && -f > Z_STRSIZE_PP(str))) {
				RETURN_FALSE;
			} else if (l > Z_STRSIZE_PP(str) || (l < 0 && -l > Z_STRSIZE_PP(str))) {
				l = Z_STRSIZE_PP(str);
			}

			if ((f + l) > Z_STRSIZE_PP(str)) {
				l = Z_STRSIZE_PP(str) - f;
			}
			if (Z_TYPE_PP(repl) == IS_ARRAY) {
				...
				if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_PP(repl), (void **) &tmp_repl, &pos_repl)) {
					...
					repl_len = Z_STRSIZE_PP(tmp_repl);
				}
			} else {
				repl_len = Z_STRSIZE_PP(repl);
			}
			result_len = Z_STRSIZE_PP(str) - l + repl_len;
			result = emalloc(result_len + 1);
			...
			memcpy((result + f + repl_len), Z_STRVAL_PP(str) + f + l, Z_STRSIZE_PP(str) - f - l);
			result[result_len] = '\0';
			RETURN_STRINGL(result, result_len, 0);
		} else {
		...
		char *str_index = NULL;
		zend_string_size str_index_len;
		ulong num_index;
		...
		while (zend_hash_get_current_data_ex(Z_ARRVAL_PP(str), (void **) &tmp_str, &pos_str) == SUCCESS) {
			...
			if (Z_TYPE_PP(repl) == IS_ARRAY) {
				if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_PP(repl), (void **) &tmp_repl, &pos_repl)) {
					...
					memcpy((result + f), Z_STRVAL_P(repl_str), Z_STRSIZE_P(repl_str));
					memcpy((result + f + Z_STRSIZE_P(repl_str)), Z_STRVAL_P(orig_str) + f + l, Z_STRSIZE_P(orig_str) - f - l);
					if(Z_TYPE_PP(tmp_repl) != IS_STRING) {
						zval_dtor(repl_str);
					}
				} else {
					...
					memcpy(result, Z_STRVAL_P(orig_str), f);
					memcpy((result + f), Z_STRVAL_P(orig_str) + f + l, Z_STRSIZE_P(orig_str) - f - l);
				}
			} else {
				result_len += Z_STRSIZE_PP(repl);

				result = emalloc(result_len + 1);

				memcpy(result, Z_STRVAL_P(orig_str), f);
				memcpy((result + f), Z_STRVAL_PP(repl), Z_STRSIZE_PP(repl));
				memcpy((result + f + Z_STRSIZE_PP(repl)), Z_STRVAL_P(orig_str) + f + l, Z_STRSIZE_P(orig_str) - f - l);
			}
...
typedef struct {
	...
	zend_string_size			m;			/* minimum pattern length */
...
static inline void php_strtr_populate_shift(PATNREPL *patterns, int patnum, int B, zend_string_size m, SHIFT_TAB *shift)
{
	int		i;
	zend_string_size	j,
			max_shift;
...
static PATNREPL *php_strtr_array_prepare_repls(zend_string_size slen, HashTable *pats, zend_llist **allocs, zend_string_size *outsize)
{
	PATNREPL		*patterns;
	HashPosition	hpos;
	zval			**entry;
	...
	*allocs = emalloc(sizeof **allocs);
	...
	for (i = 0, zend_hash_internal_pointer_reset_ex(pats, &hpos);
		...
		char	*string_key;
		zend_string_size  	string_key_len;
		ulong	num_key;
		zval	*tzv = NULL;
		...
		case HASH_KEY_IS_STRING:
			...
			L(&patterns[i].repl) = Z_STRSIZE_PP(entry);
			i++;
...
static void php_strtr_array_do_repl(STR *text, PPRES *d, zval *return_value)
{
	...
	while (pos <= lastpos) {
		...
		zend_string_size	shift	= d->shift->entries[h];
		...
		if (shift > 0) {
			...
			zend_string_size		offset_start	= d->hash->entries[h2],
...
/* {{{ php_strtr_array */
static void php_strtr_array(zval *return_value, char *str, zend_string_size slen, HashTable *pats)
{
	PPRES		*data;
	STR			text;
	PATNREPL	*patterns;
	zend_string_size			patterns_len;
	zend_llist	*allocs;
...
PHP_FUNCTION(strtr)
{
	zval **from;
	char *str, *to = NULL;
	zend_string_size str_len, to_len = 0;
...
/* {{{ php_similar_str
 */
static void php_similar_str(const char *txt1, zend_string_size len1, const char *txt2, zend_string_size len2, zend_string_size *pos1, zend_string_size *pos2, zend_string_size *max)
{
	char *p, *q;
...
/* {{{ php_similar_char
 */
static zend_string_size php_similar_char(const char *txt1, zend_string_size len1, const char *txt2, zend_string_size len2)
{
	zend_string_size sum;
	zend_string_size pos1, pos2, max;
...
PHP_FUNCTION(similar_text)
{
	...
	zval **percent = NULL;
	int ac = ZEND_NUM_ARGS();
	zend_string_size sim;
	zend_string_size t1_len, t2_len;
...
{
	char *str, *what;
	zend_string_size str_len, what_len;
	...
	Z_STRVAL_P(return_value) = php_addcslashes(str, str_len, &Z_STRSIZE_P(return_value), 0, what, what_len TSRMLS_CC);
	RETURN_STRINGL(Z_STRVAL_P(return_value), Z_STRSIZE_P(return_value), 0);
}
/* }}} */
...
/* {{{ php_addcslashes
 */
PHPAPI char *php_addcslashes(const char *str, zend_string_size length, zend_string_size *new_length, int should_free, char *what, zend_string_size wlength TSRMLS_DC)
{
	char flags[256];
...
/* {{{ php_addslashes
 */
PHPAPI char *php_addslashes(char *str, zend_string_size length, zend_string_size *new_length, int should_free TSRMLS_DC)
{
	/* maximum string length, worst case situation */
	char *new_str;
	char *source, *target;
	char *end;
	zend_string_size local_new_length;
...
{
	zend_string_size char_count = 0;
	zend_string_size replaced = 0;
	...
	Z_STRSIZE_P(result) = len + (char_count * (to_len - 1));
...
/* {{{ php_str_to_str_ex
 */
PHPAPI char *php_str_to_str_ex(char *haystack, zend_string_size length,
	char *needle, zend_string_size needle_len, char *str, zend_string_size str_len, zend_string_size *_new_length, int case_sensitivity, zend_string_size *replace_count)
{
	char *new_str;
...
/* {{{ php_str_to_str
 */
PHPAPI char *php_str_to_str(char *haystack, zend_string_size length,
	char *needle, zend_string_size needle_len, char *str, zend_string_size str_len, zend_string_size *_new_length)
{
 ...
 */
static void php_str_replace_in_subject(zval *search, zval *replace, zval **subject, zval *result, int case_sensitivity, zend_string_size *replace_count)
{
	zval		**search_entry,
				**replace_entry = NULL,
				  temp_result;
	char		*replace_value = NULL;
	zend_string_size			 replace_len = 0;

	/* Make sure we're dealing with strings. */
	convert_to_string_ex(subject);
	Z_TYPE_P(result) = IS_STRING;
	if (Z_STRSIZE_PP(subject) == 0) {
	...
	if (Z_TYPE_P(search) == IS_ARRAY) {
		...
		while (zend_hash_get_current_data(Z_ARRVAL_P(search), (void **) &search_entry) == SUCCESS) {
			...
			if (Z_STRSIZE_PP(search_entry) == 0) {
			...
			if (Z_TYPE_P(replace) == IS_ARRAY) {
				...
				if (zend_hash_get_current_data(Z_ARRVAL_P(replace), (void **)&replace_entry) == SUCCESS) {
					...
					replace_len = Z_STRSIZE_PP(replace_entry);
			...
			if (Z_STRSIZE_PP(search_entry) == 1) {
				php_char_to_str_ex(Z_STRVAL_P(result),
								...
								case_sensitivity,
								replace_count);
			} else if (Z_STRSIZE_PP(search_entry) > 1) {
				Z_STRVAL(temp_result) = php_str_to_str_ex(Z_STRVAL_P(result), Z_STRSIZE_P(result),
														   Z_STRVAL_PP(search_entry), Z_STRSIZE_PP(search_entry),
														   replace_value, replace_len, &Z_STRSIZE(temp_result), case_sensitivity, replace_count);
			}

           str_efree(Z_STRVAL_P(result));
			Z_STRVAL_P(result) = Z_STRVAL(temp_result);
			Z_STRSIZE_P(result) = Z_STRSIZE(temp_result);
		...
		if (Z_STRSIZE_P(search) == 1) {
			...
			Z_STRVAL_P(result) = php_str_to_str_ex(Z_STRVAL_PP(subject), Z_STRSIZE_PP(subject),
								Z_STRVAL_P(search), Z_STRSIZE_P(search),
								Z_STRVAL_P(replace), Z_STRSIZE_P(replace),
								&Z_STRSIZE_P(result), case_sensitivity, replace_count);
		} else {
			MAKE_COPY_ZVAL(subject, result);
		}
	}
...
static void php_hebrev(INTERNAL_FUNCTION_PARAMETERS, int convert_newlines)
{
	...
	zend_string_size block_start, block_end, block_type, block_length, i;
	long max_chars=0;
	zend_string_size begin, end, char_count, orig_begin;
	zend_string_size str_len;
	...
	*target = 0;
	...
	if (convert_newlines) {
		...
		Z_STRSIZE_P(return_value) = str_len;
...
PHP_FUNCTION(nl2br)
{
	...
	{
		zend_string_size repl_len = is_xhtml ? (sizeof("<br />") - 1) : (sizeof("<br>") - 1);
...
PHP_FUNCTION(strip_tags)
{
	...
	zend_string_size allowed_tags_len=0;
	zend_string_size str_len;
	size_t retval_len;
	...
	if (allow != NULL) {
		...
		allowed_tags_len = Z_STRSIZE_PP(allow);
	}
...
PHPAPI size_t php_strip_tags(char *rbuf, zend_string_size len, int *stateptr, char *allow, zend_string_size allow_len) /* {{{ */
{
...
PHP_FUNCTION(str_getcsv)
{
	...
	zend_string_size str_len = 0, delim_len = 0, enc_len = 0, esc_len = 0;
...
PHP_FUNCTION(str_repeat)
{
	...
	zend_string_size 		input_len;
	long 		mult;			/* Multiplier */
	char		*result;		/* Resulting string */
	zend_string_size		result_len;		/* Length of the resulting string */
...
{
	char *s1, *s2;
	zend_string_size s1_len, s2_len;
...
PHPAPI int string_natural_compare_function_ex(zval *result, zval *op1, zval *op2, zend_bool case_insensitive TSRMLS_DC) /* {{{ */
{
	...
	ZVAL_LONG(result, strnatcmp_ex(Z_STRVAL_P(op1), Z_STRSIZE_P(op1), Z_STRVAL_P(op2), Z_STRSIZE_P(op2), case_insensitive));
...
PHP_FUNCTION(substr_count)
{
	...
	int ac = ZEND_NUM_ARGS();
	int count = 0;
	zend_string_size haystack_len, needle_len;
	char *p, *endp, cmp;
...
PHP_FUNCTION(str_pad)
{
	...
	/* Helper variables */
	zend_string_size	   num_pad_chars;		/* Number of padding characters (total - input size) */
	char  *result = NULL;		/* Resulting string */
	zend_string_size	   result_len = 0;		/* Length of the resulting string */
	char  *pad_str_val = " ";	/* Pointer to padding string */
	zend_string_size    pad_str_len = 1;		/* Length of the padding string */
...
PHP_FUNCTION(sscanf)
{
	zval ***args = NULL;
	char *str, *format;
	zend_string_size str_len, format_len, result;
	int num_args = 0;
...
static void php_string_shuffle(char *str, zend_string_size len TSRMLS_DC) /* {{{ */
{
...
PHP_FUNCTION(str_shuffle)
{
	...
	if (Z_STRSIZE_P(return_value) > 1) {
		php_string_shuffle(Z_STRVAL_P(return_value), Z_STRSIZE_P(return_value) TSRMLS_CC);
	}
}
/* }}} */
...
PHP_FUNCTION(str_word_count)
{
	...
	zend_string_size str_len, char_list_len = 0;
	long word_count = 0;
	long type = 0;
...
{
	zend_string_size format_len = 0, str_len;
	char *format, *str, *p, *e;
	double value;
	zend_bool check = 0;
...
PHP_FUNCTION(str_split)
{
	...
	long split_length = 1;
	char *p;
	zend_string_size n_reg_segments;
...
PHP_FUNCTION(strpbrk)
{
	...
	zend_string_size haystack_len, char_list_len;
...
PHP_FUNCTION(substr_compare)
{
	...
	cmp_len = (len ? len : MAX(s2_len, (s1_len - offset)));
