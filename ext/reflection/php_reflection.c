...
static string *string_printf(string *str, const char *format, ...)
{
	...
	if (len) {
		register zend_string_size nlen = (str->len + len + (1024 - 1)) & ~(1024 - 1);
		if (str->alloced < nlen) {
			str->alloced = nlen;
...
static string *string_write(string *str, char *buf, zend_string_size len)
{
	register zend_string_size nlen = (str->len + len + (1024 - 1)) & ~(1024 - 1);
	if (str->alloced < nlen) {
		str->alloced = nlen;
...
static void _default_get_entry(zval *object, char *name, zend_string_size name_len, zval *return_value TSRMLS_DC) /* {{{ */
{
	zval **value;
...
#ifdef ilia_0
static void _default_lookup_entry(zval *object, char *name, zend_string_size name_len, zval **return_value TSRMLS_DC) /* {{{ */
{
	zval **value;
	...
	if (obj && Z_OBJ_HT_P(obj)->get_properties) {
		...
		if (properties && zend_hash_num_elements(properties)) {
			...
			while (zend_hash_get_current_data_ex(properties, (void **) &prop, &pos) == SUCCESS) {
				char  *prop_name;
				zend_string_size prop_name_size;
				ulong index;
		...
		if (count > 0) {
			...
			while (zend_hash_get_current_data_ex(&ce->function_table, (void **) &mptr, &pos) == SUCCESS) {
					...
					&& ((mptr->common.fn_flags & ZEND_ACC_PRIVATE) == 0 || mptr->common.scope == ce))
				{
					...
					zend_string_size key_len;
					ulong num_index;
					zend_string_size len = strlen(mptr->common.function_name);
...
static void _extension_string(string *str, zend_module_entry *module, char *indent TSRMLS_DC)
{
	...
	if (module->functions && module->functions->fname) {
		...
		while (func->fname) {
			zend_string_size fname_len = strlen(func->fname);
...
static void reflection_extension_factory(zval *object, const char *name_str TSRMLS_DC)
{
	reflection_object *intern;
	zval *name;
	zend_string_size name_len = strlen(name_str);
	char *lcname;
...
ZEND_METHOD(reflection_method, __construct)
{
	...
	zend_function *mptr;
	char *name_str, *tmp;
	zend_string_size name_len, tmp_len;
	zval ztmp;
...
ZEND_METHOD(reflection_class, getProperty)
{
	...
	zend_string_size name_len, classname_len;
