...
static void php_load_zend_extension_cb(void *arg TSRMLS_DC)
{
	...
	zend_string_size length = strlen(filename);
	...
	if (IS_ABSOLUTE_PATH(filename, length)) {
		...
		zend_string_size extension_dir_len = strlen(extension_dir);
...
int php_init_config(TSRMLS_D)
{
	...
	zend_string_size php_ini_scanned_path_len;
	char *open_basedir;
	zend_string_size free_ini_search_path = 0;
	zend_file_handle fh;
	...
	if (sapi_module.php_ini_path_override) {
		...
		zend_string_size search_path_size;
 ...
 */
PHPAPI void php_ini_activate_per_dir_config(char *path, zend_string_size path_len TSRMLS_DC)
{
	zval *tmp2;
	char *ptr;
 ...
 */
PHPAPI void php_ini_activate_per_host_config(const char *host, zend_string_size host_len TSRMLS_DC)
{
	zval *tmp;
...
/* {{{ cfg_get_entry
 */
PHPAPI zval *cfg_get_entry(const char *name, zend_string_size name_length)
{
	zval *tmp;
