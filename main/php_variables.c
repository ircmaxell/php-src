...
/* binary-safe version */
PHPAPI void php_register_variable_safe(char *var, char *strval, zend_string_size str_len, zval *track_vars_array TSRMLS_DC)
{
	zval new_entry;
	assert(strval != NULL);
...
PHPAPI void php_register_variable_ex(char *var_name, zval *val, zval *track_vars_array TSRMLS_DC)
{
	...
	char *index;
	char *var, *var_orig;
	zend_string_size var_len, index_len;
	...
	if (is_array) {
		...
		while (1) {
			char *index_s;
			zend_string_size new_idx_len = 0;
...
last_value:
		if ((val = memchr(s, '=', (p - s)))) { /* have a value */
			zend_string_size val_len, new_val_len;
...
void _php_import_environment_variables(zval *array_ptr TSRMLS_DC)
{
	char buf[128];
	char **env, *p, *t = buf;
	zend_string_size alloc_size = sizeof(buf);
	zend_string_size nlen; /* ptrdiff_t is not portable */
...
zend_bool php_std_auto_global_callback(char *name, zend_string_size name_len TSRMLS_DC)
{
	zend_printf("%s\n", name);
...
static zend_bool php_auto_globals_create_server(const char *name, zend_string_size name_len TSRMLS_DC);
static zend_bool php_auto_globals_create_env(const char *name, zend_string_size name_len TSRMLS_DC);
static zend_bool php_auto_globals_create_request(const char *name, zend_string_size name_len TSRMLS_DC);
...
static zend_bool php_auto_globals_create_get(const char *name, zend_string_size name_len TSRMLS_DC)
{
	zval *vars;
...
static zend_bool php_auto_globals_create_post(const char *name, zend_string_size name_len TSRMLS_DC)
{
	zval *vars;
...
static zend_bool php_auto_globals_create_cookie(const char *name, zend_string_size name_len TSRMLS_DC)
{
	zval *vars;
...
static zend_bool php_auto_globals_create_files(const char *name, zend_string_size name_len TSRMLS_DC)
{
	zval *vars;
