...
#define CALCULATE_LITERAL_HASH(num) do { \
		if (IS_INTERNED(Z_STRVAL(CONSTANT(num)))) { \
			...
			Z_HASH_P(&CONSTANT(num)) = zend_hash_func(Z_STRVAL(CONSTANT(num)), Z_STRSIZE(CONSTANT(num))+1); \
		} \
	} while (0)
...
static void build_runtime_defined_function_key(zval *result, const char *name, zend_string_size name_length TSRMLS_DC) /* {{{ */
{
	char char_pos_buf[32];
	zend_string_size char_pos_len;
	const char *filename;
...
{
	char **pp, *p;
	zend_string_size length = strlen(new_compiled_filename);
...
static int lookup_cv(zend_op_array *op_array, char* name, zend_string_size name_len, ulong hash TSRMLS_DC) /* {{{ */
{
	int i = 0;
...
int zend_add_const_name_literal(zend_op_array *op_array, const zval *zv, int unqualified TSRMLS_DC) /* {{{ */
{
	...
	char *name, *tmp_name;
	const char *ns_separator;
	zend_string_size name_len, ns_len;
	zval c;
...
void zend_do_begin_function_declaration(znode *function_token, znode *function_name, int is_method, int return_reference, znode *fn_flags_znode TSRMLS_DC) /* {{{ */
{
	...
	zend_string_size name_len = function_name->u.constant.value.str.len;
...
void zend_do_receive_arg(zend_uchar op, znode *varname, const znode *offset, const znode *initialization, znode *class_type, zend_uchar pass_by_reference TSRMLS_DC) /* {{{ */
{
	...
	if (class_type->op_type == IS_CONST &&
	    ...
	    Z_STRSIZE(class_type->u.constant) == 0) {
	...
	if (zend_is_auto_global_quick(Z_STRVAL(varname->u.constant), Z_STRSIZE(varname->u.constant), 0 TSRMLS_CC)) {
	...
	if (last_op->opcode == ZEND_FETCH_OBJ_R) {
		if (last_op->op2_type == IS_CONST) {
			...
			if (!IS_INTERNED(Z_STRVAL(name))) {
				Z_STRVAL(name) = estrndup(Z_STRVAL(name), Z_STRSIZE(name));
			}
...
void zend_do_fetch_class(znode *result, znode *class_name TSRMLS_DC) /* {{{ */
{
	...
	if (class_name->op_type == IS_CONST &&
	    ...
	    Z_STRSIZE(class_name->u.constant) == 0) {
		/* Usage of namespace as class name not in namespace */
		zval_dtor(&class_name->u.constant);
		zend_error(E_COMPILE_ERROR, "Cannot use 'namespace' as a class name");
		return;
...
static zend_bool zend_do_perform_implementation_check(const zend_function *fe, const zend_function *proto TSRMLS_DC) /* {{{ */
{
	...
	for (i=0; i < proto->common.num_args; i++) {
		...
		if (fe->common.arg_info[i].class_name) {
			...
			zend_string_size fe_class_name_len, proto_class_name_len;
...
static char * zend_get_function_declaration(zend_function *fptr TSRMLS_DC) /* {{{ */
{
	...
	{
		zend_string_size name_len = strlen(fptr->common.function_name);
		...
		for (i = 0; i < fptr->common.num_args;) {
			if (arg_info->class_name) {
				...
				*(offset++) = ' ';
				...
				zend_string_size type_name_len;
...
static void zend_add_trait_method(zend_class_entry *ce, const char *name, const char *arKey, zend_string_size nKeyLength, zend_function *fn, HashTable **overriden TSRMLS_DC) /* {{{ */
{
		...
		while ((cur_precedence = ce->trait_precedences[i])) {
			...
			if (cur_precedence->exclude_from_classes) {
				...
				while (cur_precedence->exclude_from_classes[j]) {
					...
					zend_string_size name_length = strlen(class_name);
...
static zend_class_entry* find_first_definition(zend_class_entry *ce, size_t current_trait, const char* prop_name, zend_string_size prop_name_length, ulong prop_hash, zend_class_entry *coliding_ce) /* {{{ */
{
	size_t i;
...
static void zend_do_traits_property_binding(zend_class_entry *ce TSRMLS_DC) /* {{{ */
{
	...
	zend_string_size prop_name_length;
	ulong prop_hash;
...
ZEND_API void zend_mangle_property_name(char **dest, zend_string_size *dest_length, const char *src1, zend_string_size src1_length, const char *src2, zend_string_size src2_length, int internal) /* {{{ */
{
	char *prop_name;
	zend_string_size prop_name_length;
...
static zend_string_size zend_strnlen(const char* s, zend_string_size maxlen) /* {{{ */
{
	zend_string_size len = 0;
	while (*s++ && maxlen--) len++;
	return len;
}
/* }}} */

ZEND_API int zend_unmangle_property_name_ex(const char *mangled_property, zend_string_size len, const char **class_name, const char **prop_name, zend_string_size *prop_len) /* {{{ */
{
	zend_string_size class_name_len;
...
void zend_do_declare_property(const znode *var_name, const znode *value, zend_uint access_type TSRMLS_DC) /* {{{ */
{
	...
	char *comment = NULL;
	zend_string_size comment_len = 0;
	...
	if (offset) {
		switch (offset->u.constant.type & IS_CONSTANT_TYPE_MASK) {
			case IS_CONSTANT:
				...
				zend_symtable_update(result->u.constant.value.ht, Z_STRVAL(offset->u.constant), Z_STRSIZE(offset->u.constant)+3, &element, sizeof(zval *), NULL);
...
zend_bool zend_is_auto_global_quick(const char *name, zend_string_size name_len, ulong hashval TSRMLS_DC) /* {{{ */
{
...
zend_bool zend_is_auto_global(const char *name, zend_string_size name_len TSRMLS_DC) /* {{{ */
{
	return zend_is_auto_global_quick(name, name_len, 0 TSRMLS_CC);
}
/* }}} */

int zend_register_auto_global(const char *name, zend_string_size name_len, zend_bool jit, zend_auto_global_callback auto_global_callback TSRMLS_DC) /* {{{ */
{
	zend_auto_global auto_global;
...
int zend_get_class_fetch_type(const char *class_name, zend_string_size class_name_len) /* {{{ */
{
...
ZEND_API const char* zend_get_compiled_variable_name(const zend_op_array *op_array, zend_uint var, zend_string_size* name_len) /* {{{ */
{
	if (name_len) {
...
ZEND_API zend_string_size zend_dirname(char *path, zend_string_size len)
{
	register char *end = path + len - 1;
	zend_string_size len_adjust = 0;
	...
	*(end+1) = '\0';
	...
	return (zend_string_size)(end + 1 - path) + len_adjust;
}
/* }}} */
