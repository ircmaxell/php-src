...
ZEND_METHOD(error_exception, __construct)
{
	...
	zval  *object, *previous = NULL;
	int    argc = ZEND_NUM_ARGS();
	zend_string_size message_len, filename_len;
...
static int _build_trace_args(zval **arg TSRMLS_DC, int num_args, va_list args, zend_hash_key *hash_key) /* {{{ */
{
	...
	len = va_arg(args, zend_string_size*);
	...
	switch (Z_TYPE_PP(arg)) {
		...
		case IS_LONG: {
			...
			zend_string_size l_tmp = zend_sprintf(s_tmp, "%ld", lval);  /* SAFE */
...
ZEND_METHOD(exception, getTraceAsString)
{
	zval *trace;
	char *res, **str, *s_tmp;
	zend_string_size res_len = 0, *len = &res_len;
	int num = 0;
...
int zend_spprintf(char **message, zend_string_size max_len, char *format, ...) /* {{{ */
{
	va_list arg;
	zend_string_size len;
