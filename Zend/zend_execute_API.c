	...
	if (IS_CONSTANT_VISITED(p)) {
		...
		if (!zend_get_constant_ex(p->value.str.val, p->value.str.len, &const_value, scope, Z_REAL_TYPE_P(p) TSRMLS_CC)) {
			...
			if ((colon = (char*)zend_memrchr(Z_STRVAL_P(p), ':', Z_STRSIZE_P(p)))) {
				...
				zend_string_size actual_len = Z_STRSIZE_P(p);
...
ZEND_API int zend_lookup_class_ex(const char *name, zend_string_size name_length, const zend_literal *key, int use_autoload, zend_class_entry ***ce TSRMLS_DC) /* {{{ */
{
	zval **args[1];
...
ZEND_API int zend_lookup_class(const char *name, zend_string_size name_length, zend_class_entry ***ce TSRMLS_DC) /* {{{ */
{
	return zend_lookup_class_ex(name, name_length, NULL, 1, ce TSRMLS_CC);
}
/* }}} */

ZEND_API int zend_eval_stringl(char *str, zend_string_size str_len, zval *retval_ptr, char *string_name TSRMLS_DC) /* {{{ */
{
	zval pv;
...
ZEND_API int zend_eval_stringl_ex(char *str, zend_string_size str_len, zval *retval_ptr, char *string_name, int handle_exceptions TSRMLS_DC) /* {{{ */
{
	int result;
...
zend_class_entry *zend_fetch_class(const char *class_name, zend_string_size class_name_len, int fetch_type TSRMLS_DC) /* {{{ */
{
...
zend_class_entry *zend_fetch_class_by_name(const char *class_name, zend_string_size class_name_len, const zend_literal *key, int fetch_type TSRMLS_DC) /* {{{ */
{
	zend_class_entry **pce;
	int use_autoload = (fetch_type & ZEND_FETCH_CLASS_NO_AUTOLOAD) == 0;
