...
{
	zend_string_size length = Z_STRSIZE_P(op1) + 1;
	char *buf;
...
{
	zend_string_size length = Z_STRSIZE_P(op1) + Z_STRSIZE_P(op2);
	char *buf;
...
ZEND_API int concat_function(zval *result, zval *op1, zval *op2 TSRMLS_DC) /* {{{ */
{
	...
	if (result==op1 && !IS_INTERNED(Z_STRVAL_P(op1))) {	/* special case, perform operations on result */
		zend_string_size res_len = Z_STRSIZE_P(op1) + Z_STRSIZE_P(op2);

		if (Z_STRSIZE_P(result) < 0 || (Z_STRSIZE_P(op1) + Z_STRSIZE_P(op2)) < Z_STRSIZE_P(op1)) {
...
{
	int carry=0;
	zend_string_size pos=Z_STRSIZE_P(str)-1;
...
ZEND_API int zend_binary_strcasecmp_l(const char *s1, zend_string_size len1, const char *s2, zend_string_size len2) /* {{{ */
{
	zend_string_size len;
	int c1, c2;
