...
ZEND_API void zend_register_long_constant(const char *name, zend_string_size name_len, long lval, int flags, int module_number TSRMLS_DC)
{
	zend_constant c;
...
ZEND_API void zend_register_double_constant(const char *name, zend_string_size name_len, double dval, int flags, int module_number TSRMLS_DC)
{
	zend_constant c;
...
ZEND_API void zend_register_stringl_constant(const char *name, zend_string_size name_len, char *strval, zend_string_size strlen, int flags, int module_number TSRMLS_DC)
{
	zend_constant c;
...
ZEND_API void zend_register_string_constant(const char *name, zend_string_size name_len, char *strval, int flags, int module_number TSRMLS_DC)
{
	zend_register_stringl_constant(name, name_len, strval, strlen(strval), flags, module_number TSRMLS_CC);
}

static int zend_get_special_constant(const char *name, zend_string_size name_len, zend_constant **c TSRMLS_DC)
{
	int ret;
	...
	if (!EG(in_execution)) {
		...
		if (EG(scope) && EG(scope)->name) {
			zend_string_size const_name_len;
			char *const_name;
...
ZEND_API int zend_get_constant(const char *name, zend_string_size name_len, zval *result TSRMLS_DC)
{
	zend_constant *c;
	int retval = 1;
	char *lookup_name;
...
ZEND_API int zend_get_constant_ex(const char *name, zend_string_size name_len, zval *result, zend_class_entry *scope, ulong flags TSRMLS_DC)
{
	zend_constant *c;
	int retval = 1;
	const char *colon;
	...
	if ((colon = zend_memrchr(name, ':', name_len)) &&
	    colon > name && (*(colon - 1) == ':')) {
		zend_string_size class_name_len = colon - name - 1;
		zend_string_size const_name_len = name_len - class_name_len - 2;
	...
	if ((colon = zend_memrchr(name, '\\', name_len)) != NULL) {
		...
		zend_string_size prefix_len = colon - name;
		zend_string_size const_name_len = name_len - prefix_len - 1;
		const char *constant_name = colon + 1;
		char *lcname;
		int found_const = 0;
